...
def create_batches(desc_list, photo_features, tokenizer, max_len, vocab_size=7377):
...
def train():
    ...
    train = task3.load_image_names(filename)
    print('Dataset: %d' % len(train))
    train_captions = util.load_clean_captions('../task3/descriptions.txt', train)
    print('Captions: train number=%d' % len(train_captions))
    # photo features
    train_features = util.load_photo_features('../task2/features.pkl', train)
    ...
    max_len = util.get_max_length(train_captions)-2 # remove the 2 number of image name
